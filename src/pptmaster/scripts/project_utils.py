...
def parse_project_name(dir_name: str) -> Dict[str, str]:
    ...
    dir_name_stripped = dir_name

    # Strip leading dot for hidden project directories (.name_format_YYYYMMDD)
    if dir_name_lower.startswith('.'):
        dir_name_lower = dir_name_lower[1:]
        dir_name_stripped = dir_name[1:]
    ...
    if full_match:
        ...
        if normalized_format in CANVAS_FORMATS:
            ...
            result['name'] = dir_name_stripped[:len(full_match.group('name'))]
            return result
...
def get_project_info(project_path: str) -> Dict:
    ...
    source_ref_file = project_path / 'source_ref.json'
    info['has_source'] = (
        legacy_source_file.exists()
        or sources_dir.exists()
        or source_ref_file.exists()
    )

    if sources_dir.exists():
        info['source_count'] = len([p for p in sources_dir.iterdir() if p.is_file()])

    if source_ref_file.exists():
        info['create_mode'] = True
...
def find_all_projects(base_dir: str) -> List[Path]:
    ...
    for item in base_path.iterdir():
        if not item.is_dir():
            continue
        # Skip hidden dirs unless they match the hidden project pattern
        if item.name.startswith('.') and not re.match(r'^\..+_[a-z0-9]+_\d{8}$', item.name):
            continue
        ...
        has_source_ref = (item / 'source_ref.json').exists()

        if has_svg_output or has_spec or has_source_ref:

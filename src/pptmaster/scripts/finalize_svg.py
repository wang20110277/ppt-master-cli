...
try:
    from pptmaster.scripts.svg_finalize.crop_images import process_svg_images as crop_images_in_svg
    from pptmaster.scripts.svg_finalize.embed_icons import process_svg_file as embed_icons_in_file
    from pptmaster.scripts.svg_finalize.embed_images import embed_images_in_svg
    from pptmaster.scripts.svg_finalize.fix_image_aspect import fix_image_aspect_in_svg
except ImportError:
...
def _import_svg_finalize(submod: str, name: str):
    """Import from svg_finalize subpackage (package or standalone mode)."""
    try:
        mod = __import__(f"pptmaster.scripts.svg_finalize.{submod}", fromlist=[name])
        return getattr(mod, name)
    except ImportError:
        mod = __import__(f"svg_finalize.{submod}", fromlist=[name])
        return getattr(mod, name)
...
def process_flatten_text(svg_file: Path, verbose: bool = False) -> bool:
    ...
    try:
        flatten_text_with_tspans = _import_svg_finalize("flatten_tspan", "flatten_text_with_tspans")  # noqa: fallback after import error
...
def process_rounded_rect(svg_file: Path, verbose: bool = False) -> int:
    ...
    try:
        process_svg = _import_svg_finalize("svg_rect_to_path", "process_svg")
...
def finalize_project(
    ...
    try:
        from pptmaster._paths import get_icons_dir
